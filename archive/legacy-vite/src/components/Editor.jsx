...
const Editor = ({ socketRef, roomId, tabId, onCodeChange, onSelectionToChat, externalCode, acceptRemote }) => {
    ...
    useEffect(() => {
        ...
        return () => {
            try { editorRef.current?.toTextArea(); } catch {}
            editorRef.current = null;
        };
    }, []);

    useEffect(() => { if (editorRef.current) editorRef.current.setOption('mode', { name: lang }); }, [lang]);
    useEffect(() => { if (editorRef.current) editorRef.current.setOption('theme', editorTheme); }, [editorTheme]);
    ...
    useEffect(() => {
        ...
        const handler = ({ code, tabId: incomingTabId }) => {
